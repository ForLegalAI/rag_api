...
- `DOCX_TEXT_TRACK_CHANGES`: (Optional) pandoc `--track-changes` mode for the `.docx` `/text` path: `all` (default; keep insertions/deletions and emit comments), `accept` (final text), or `reject` (original text).
