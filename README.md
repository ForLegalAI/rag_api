...
> - **Richer DOCX text extraction** — `.docx` files routed to `/text` are converted with [pandoc](https://pandoc.org/), preserving tracked changes, comments, and (optionally) headers/footers. Tables are written as compact pipe tables and the Markdown is cleaned of extraction noise, so a bilingual contract laid out as one long table no longer extracts at several times its content size. Configurable via `DOCX_TEXT_USE_PANDOC`, `DOCX_TEXT_TRACK_CHANGES`, `DOCX_TEXT_INCLUDE_HEADERS_FOOTERS`, `DOCX_TEXT_TABLE_STYLE`, `DOCX_TEXT_CLEANUP`, `DOCX_TEXT_STRIP_HEADING_ANCHORS` and `DOCX_TEXT_DROP_TOC`.
...
- `DOCX_TEXT_INCLUDE_HEADERS_FOOTERS`: (Optional) Boolean. When "True" (default), `.docx` headers/footers (matter numbers, "PRIVILEGED & CONFIDENTIAL", "DRAFT", etc.) — which pandoc drops — are extracted via python-docx and prepended to the `/text` output. Headers/footers holding nothing but a page number ("4", "Page 4 of 10") are skipped.
- `DOCX_TEXT_TABLE_STYLE`: (Optional) `compact` (default) or `grid`. Pandoc's own Markdown writer pads every cell of every row out to the width of the longest line in its column; with `--wrap=none` a single long clause can therefore inflate a whole contract table many times over, most of the output being spaces and `+---+` rules. `compact` disables grid/multiline/simple tables, so pandoc writes unpadded pipe tables and falls back to an HTML `<table>` only for cells holding block content. On bilingual contract tables this cuts the extracted text by 55-82% with no loss of content, tracked changes included. Set `grid` to restore pandoc's default table output; pair it with `DOCX_TEXT_CLEANUP=False`, since grid tables encode their columns as character positions and a cleanup that shortens a row shifts its `|` delimiters off the `+---+` rule (the service warns at startup if you set one without the other); reproducing the pre-fix output byte for byte also needs `DOCX_TEXT_CLEANUP=False` and `DOCX_TEXT_STRIP_HEADING_ANCHORS=False`, which are independent settings.
- `DOCX_TEXT_CLEANUP`: (Optional) Boolean. When "True" (default), the extracted Markdown is stripped of noise that carries no content: table columns that are empty in every row, empty tracked-change spans (a deleted space still costs ~70 bytes of author/date metadata), runs of blank lines from Word spacer paragraphs, and invisible characters (NBSP, soft hyphen, zero-width). Tracked changes that carry text keep their author and date untouched, and comments are preserved.
- `DOCX_TEXT_STRIP_HEADING_ANCHORS`: (Optional) Boolean. When "True" (default), the anchors pandoc derives from Word bookmarks (`# Heading {#_Toc123456789}`) are omitted — they are Word-internal TOC/cross-reference ids, not document content.
- `DOCX_TEXT_DROP_TOC`: (Optional) Boolean. When "True", a Word-generated table of contents (the block of links to `#_Toc` bookmarks) is dropped from the extracted text. Defaults to "False", since in some documents the TOC is the only outline of the agreement.

> **Upgrading:** these defaults change what `/text` returns for `.docx` files — tables come back as pipe tables (or an HTML `<table>` for cells holding block content) instead of padded grid tables, and the cleanup pass runs. Anything downstream that depends on the previous Markdown shape byte for byte — a prompt template keyed on grid tables, or a stored extraction being compared against a fresh one — sees different text. Set `DOCX_TEXT_TABLE_STYLE=grid`, `DOCX_TEXT_CLEANUP=False` and `DOCX_TEXT_STRIP_HEADING_ANCHORS=False` to keep the old output.
...
- `IMAGE_OCR_MAX_PAGES`: (Optional) Max number of frames/pages of a single image upload (e.g. animated GIF or large multi-page TIFF) sent to OCR; frames beyond the cap are skipped with a warning. Default value is `100`.
